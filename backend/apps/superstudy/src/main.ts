  ...
  if (process.env.NODE_ENV === 'production') {
    if (cluster.isPrimary) {
      ...
      await app.listen(PORT, () => {
        Logger.log(`[Super Study] Server is now running in production at port ${PORT}`); // unless $PORT is undefined, in which case you're listening to 8081.
      });
    ...
    await app.listen(PORT, () => {
      Logger.log(`[Super Study] Server is now running in development at port ${PORT}`); // unless $PORT is undefined, in which case you're listening to 8081.
    });
  }
}
bootstrap();
...
// import helmet from 'helmet';
// import * as trimRequest from 'trim-request';
// // import { SuperLMSModule } from './superlms.module';
// import { SuperStudyModule } from './superstudy.module';
