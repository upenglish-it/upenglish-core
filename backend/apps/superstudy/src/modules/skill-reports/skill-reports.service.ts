...
@Injectable()
export class SkillReportsService {
  constructor(
    @InjectModel(SSTSkillReports)
    private readonly model: ReturnModelType<typeof SSTSkillReports>,
  ) {}
  ...
  async checkLock(field: string, assignmentId: string) {
    if (!field || !assignmentId) return [];
    const docs = await this.model.find({ [field]: assignmentId, status: 'sent' }).lean();
    return docs.map(d => ({ id: d._id, ...d }));
  }
