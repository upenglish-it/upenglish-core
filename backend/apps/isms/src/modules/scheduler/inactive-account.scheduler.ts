...
@Queue('inactive-account-scheduler')
export class InactiveAccountScheduler {
  constructor(
    @InjectQueue('inactive-account-scheduler') private readonly automationScheduleAction: Agenda,

    @InjectModel(Accounts) private readonly accounts: ReturnModelType<typeof Accounts>,
  ) {
    this.automationScheduleAction.define('inactive-account-scheduler', (job: Job) => this.definedAgenda(job));
    automationScheduleAction.on('start', (job: Job) => this.startAgenda(job));
    automationScheduleAction.on('start:inactive-account-scheduler', (job: Job) => this.startAgenda(job));
    automationScheduleAction.on('success:inactive-account-scheduler', (job: Job) => this.successAgenda(job));
    automationScheduleAction.on('complete:inactive-account-scheduler', (job: Job) => this.completedAgenda(job));
    automationScheduleAction.on('fail:inactive-account-scheduler', (err: any, job: Job) => this.failAgenda(err, job));
  }
  ...
  public async processAction(data: IActionData): Promise<IResponseHandlerParams> {
    try {
      ...
      if (data.action === 'to-be-inactive') {
        // dateTime = DateTime.now().plus({ days: 40 }).toJSDate(); // inactive student after 40days
        dateTime = DateTime.now().plus({ seconds: 15 }).toJSDate(); // inactive student after 40days
      }

      await this.automationScheduleAction.schedule(dateTime, 'inactive-account-scheduler', data);
...
interface IActionData {
  action: 'to-be-active' | 'to-be-inactive';
  accountId: string;
}
