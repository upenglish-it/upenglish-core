...
interface MovableDragNDropTextboxBlotParams {
  text: string;
  participantAnswerId: string;
}

export class MovableDragNDropTextboxBlot extends Inline {
  static blotName = "slms-movable-drag-n-drop-textbox";
  static tagName = "input";
  static className = "movable-drag-n-drop-textbox";

  static create(params: MovableDragNDropTextboxBlotParams) {
    const node: HTMLElement = super.create();
    node.setAttribute("draggable", "true");
    node.setAttribute("contenteditable", "false");
    node.setAttribute("participant-answer-id", params.participantAnswerId);
    node.style.minWidth = "100px !important"; // Ensure minimum width
    ...
    node.ondrop = (e): void => {
      e.preventDefault();
      const transferredData = JSON.parse(e.dataTransfer?.getData("text/plain")) as MovableDragNDropTextboxBlotParams;
