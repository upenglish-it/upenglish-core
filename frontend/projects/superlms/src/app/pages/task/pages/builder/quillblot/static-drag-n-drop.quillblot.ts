...
interface StaticDragNDropBlotParams {
  text: string;
  participantAnswerId: string;
}

export class StaticDragNDropBlot extends Inline {
  static blotName = "slms-static-drag-n-drop";
  static tagName = "span";
  static className = "static-drag-n-drop";

  static create(params: StaticDragNDropBlotParams) {
    ...
    node.ondrop = (e): void => {
      e.preventDefault();
      const transferredData = JSON.parse(e.dataTransfer?.getData("text/plain")) as StaticDragNDropBlotParams;
