export const environment = {
  production: false,
  appName: "UPE",
  awsS3Path: "https://hrclap.s3.amazonaws.com/prod",
  awsS3UploadedCVPath: "uploaded-cv",
  lssk: "lssk",
  apiUrl: "https://api.upenglishvietnam.com/api/v1",
  socketIOUrl: "https://api.upenglishvietnam.com",

  ssoApiUrl: "http://localhost:20231/api/v1",
  recruitmentApiUrl: "http://localhost:20232/api/v1",
  recruitmentSocketIO: "http://localhost:20232",
  bigDataApiUrl: "http://localhost:20233/api/v1/recruitment",
  firebase: {
    // apiKey: "AIzaSyAfCAlrwP15JQaHDQIVAE7ja4VWzLeHbC8",
    // authDomain: "hr-clap-aa6ef.firebaseapp.com",
    // projectId: "hr-clap-aa6ef",
    // storageBucket: "hr-clap-aa6ef.appspot.com",
    // messagingSenderId: "608874872311",
    // appId: "1:608874872311:web:223c3c0b835904642102f2",
    // measurementId: "G-ZEME1N486Z",
    apiKey: "AIzaSyBFsNTXTvjU9rmcatgUPwKrvxR1l-CSU4s",
    authDomain: "isms-up.firebaseapp.com",
    projectId: "isms-up",
    storageBucket: "isms-up.appspot.com",
    messagingSenderId: "937149398504",
    appId: "1:937149398504:web:fdddac31b5280897fc785d",
    vapidKey: "BMMw6hpDLUA1zI0d_2XLb3bS6hdQIClgMU6iXtfJGz-mSKBsl1ygLM3J8U2hY71mGISQUxXqWBxnYDBq268RR34",
  },
  superLMS: "http://superlms.upenglishvietnam.com",
};
// BMMw6hpDLUA1zI0d_2XLb3bS6hdQIClgMU6iXtfJGz-mSKBsl1ygLM3J8U2hY71mGISQUxXqWBxnYDBq268RR34
// 5kYgKwlc63e4YjTtI08Mbltd0LZoTeHgdZ5flOGUHWU
