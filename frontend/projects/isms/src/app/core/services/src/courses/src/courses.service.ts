...
export class CoursesService {
  ...
  public create(body: { name: number; price: number; material: number; hourlyMonthlyPrice: number; hourlyPackagePrice: number }): Observable<IAPIResponse> {
    return this.apiService.post(`${this.#apiUrl}`, body);
  }

  public updateById(body: { name: number; price: number; material: number; hourlyMonthlyPrice: number; hourlyPackagePrice: number }, id: string): Observable<IAPIResponse> {
