import { ChangeDetectorRef, ElementRef, Injectable, Input, OnDestroy, OnInit, ViewChild } from "@angular/core";
...
export const SetSectionsInSectionItems = (sections: Array<Section>, minRowHeight: number): Array<SectionItem> => {
  const sectionItems = new Array<SectionItem>();

  sections.forEach((section) => {
    const perSectionItem = new SectionItem();
    perSectionItem.section = section;
    perSectionItem.minRowHeight = minRowHeight;
    sectionItems.push(perSectionItem);
  });
...
// @Injectable()
// export class CommonScheduler implements OnInit, OnDestroy {
...
//   constructor(private changeDetector: ChangeDetectorRef, private service: NgxTimeSchedulerService) {
...
//           this.currentPeriod.timeFrameHeadersTooltip && this.currentPeriod.timeFrameHeadersTooltip[index] ? now.locale(this.locale).format(this.currentPeriod.timeFrameHeadersTooltip[index]) : "";
