...
import { IAccount, IBranch, ICountry, INumberCountry, ISource, ITag, ITimezone, NumberTypes } from "@isms-core/interfaces";
...
import { NGRXService, StudentsService, TemplatesSourceService, TemplatesTagService } from "@isms-core/services";
...
@Component({
  selector: "isms-add-student-manually-modal",
  templateUrl: "./add-student-manually-modal.component.html",
  imports: [
    NgIf,
    NgFor,
    ReactiveFormsModule,
...
export class AddStudentManuallyModalComponent implements OnInit, OnDestroy {
  ...
  public loadingCreateStudentButton: boolean = false;
  ...
  public setTemporaryData(): void {
    this.createStudentFormGroup.get("firstName").setValue("John");
    this.createStudentFormGroup.get("lastName").setValue("Doe");
    ...
    this.createStudentFormGroup.get("gender").setValue("male");
    this.createStudentFormGroup.get("birthDate").setValue("1994-01-01");
    const addressFormGroup = this.createStudentFormGroup.get("address") as FormGroup;
  ...
  public toggle(): void {
    ...
    this.createStudentFormGroup.reset({ branches: [] });
    this.emailAddressesFormArray.clear();
  ...
  public onCreate(): void {
    this.createStudentFormGroup.markAllAsTouched();

    this.loadingCreateStudentButton = true;

    console.log("this.createStudentFormGroup", this.createStudentFormGroup.value);

    console.log(this.createStudentFormGroup.valid);
    console.log(this.createStudentFormGroup);
    lastValueFrom(this.studentService.create(this.createStudentFormGroup.value)).then((res) => {
      this.loadingCreateStudentButton = false;
      const notificationTitle = "Create Student";
      if (res.success) {
  ...
  public addContactNumber(): void {
    this.contactNumbersFormArray.push(
      new FormGroup({
        type: new FormControl(this.contactNumberTypes[0].value, []),
        countryCallingCode: new FormControl(NumberCountries[0].countryCallingCode, [Validators.required]),
        number: new FormControl(null, [Validators.required]),
      })
    );
