...
export default function AdminTeacherTopicsPage() {
    ...
    const filteredTopics = topics.filter(t => {
        ...
        const teacher = teacherMap[t.teacherId] || {};
        return (
    ...
    async function handleConfirmDelete() {
        ...
        setIsDeleting(true);
        try {
            await deleteAdminTeacherTopic(topicToDelete.id);
            setTopics(topics.filter(t => t.id !== topicToDelete.id));
            setAlertMessage({ type: 'success', text: 'Đã xóa bài học thành công!' });
        } catch (error) {
            setAlertMessage({ type: 'error', text: 'Lỗi khi xóa bài học: ' + error.message });
        }
        setIsDeleting(false);
    ...
    async function openShareModal(resource, type) {
        ...
        try {
            const [entities, groupsData, assignments] = await Promise.all([
                ...
                getGroups(),
                type !== 'teacher_topic_folder' ? getAssignmentsForTopic(resource.id) : Promise.resolve([])
            ]);
    ...
    const relevantTeacherIds = new Set([
        ...filteredTopics.map(t => t.teacherId),
    ...
    [...relevantTeacherIds].forEach(teacherId => {
        ...
        const tfIds = new Set(teacherFols.flatMap(f => f.topicIds || []));
            ...
            <div className="admin-card">
                {teacherGroupEntries.length === 0 ? (
                    <div className="admin-empty-state">
                        ...
                        <p>Không có bài học hoặc folder nào được tìm thấy.</p>
                    </div>
                ) : (
                        ...
                        <table className="admin-table">
                            <thead>
                                <tr>
                                    ...
                                    <th>Tên cấu trúc (Giáo viên / Folder / Bài học)</th>
                            ...
                            <tbody>
                                {teacherGroupEntries.map(([teacherId, groupData]) => {
                                    const { teacher, folders: tFolders, unassignedTopics: tTopics } = groupData;
                                        ...
                                        <React.Fragment key={teacherId}>
                                            ...
                                            <tr className="table-row-folder" style={{ backgroundColor: '#f8fafc' }}>
                                                ...
                                                <td>
                                                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                                        <span style={{ fontSize: '0.8rem', background: '#f0fdf4', color: '#16a34a', padding: '2px 8px', borderRadius: '4px', fontWeight: 700, border: '1px solid #bbf7d0' }}>Tổng bài: {topics.filter(t => t.teacherId === teacherId).length}</span>
                                                    </div>
                                                </td>
                                            ...
                                            {isTeacherExpanded && (
                                                <>
                                                                        ...
                                                                        folderTopics.map(topic => (
                                                                            <tr key={topic.id} className="table-row-nested" style={{ backgroundColor: '#fafafa' }}>
                                                                                ...
                                                                                <td data-label="Tên mục">
                                                                                    <div className="admin-topic-cell" style={{ paddingLeft: '56px' }}>
                                                                                        <div className="admin-topic-icon" style={{ background: `${topic.color || '#3b82f6'}15`, color: topic.color || '#3b82f6', width: '28px', height: '28px', fontSize: '0.8rem' }}>
                                                                                            {topic.icon || '📚'}
                                                                                        </div>
                                                                                        <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                                                                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', gap: '10px' }}>
                                                                                                <div className="admin-topic-name" style={{ fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                                                                    {topic.name}
                                                                                                </div>
                                                                                                <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                                                                                                    ...
                                                                                                    {topic.collaboratorIds?.length > 0 && topic.collaboratorIds.map(uid => {
                                                                                                        const name = teacherMap[uid]?.displayName || (topic.collaboratorNames && topic.collaboratorNames[uid]) || 'Giáo viên';
                                                                                                        return (
                                                                                ...
                                                                                <td data-label="Mô tả">
                                                                                    <div className="admin-text-truncate" style={{ maxWidth: '250px', fontSize: '0.85rem' }}>
                                                                                        {topic.description || <span className="admin-text-muted italic">Không có mô tả</span>}
                                                                                    </div>
                                                                                </td>
                                                                                <td className="text-right">
                                                                                    <div className="admin-table-actions">
                                                                                        ...
                                                                                        <button className="admin-action-btn" onClick={() => openShareModal(topic, 'teacher_topic')} title="Chia sẻ">
                                                                                            <Share2 size={14} />
                                                                                        </button>
                                                                                        <button className="admin-action-btn" onClick={() => openEditTopicForm(topic)} title="Sửa thông tin chủ đề">
                                                                                            <Edit size={14} />
                                                                                        </button>
                                                                                        <button className="admin-action-btn danger" onClick={() => setTopicToDelete(topic)} title="Xóa chủ đề">
                                                                                            <Trash2 size={14} />
                                                                                        </button>
                                                                                    </div>
                                                                                </td>
                                                    ...
                                                    {/* Render Teacher Unassigned Topics */}
                                                    {tTopics.length > 0 && (
                                                        <>
                                                            <tr className="admin-unassigned-header">
                                                                ...
                                                                <td colSpan="3" style={{ paddingLeft: '48px' }}>
                                                                    <div className="admin-unassigned-label" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '6px 12px', background: '#fef3c7', color: '#b45309', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 600, marginTop: '8px', marginBottom: '8px' }}>
                                                                        ...
                                                                        Bài học chưa có Folder ({tTopics.length})
                                                                    </div>
                                                                </td>
                                                            </tr>
                                                            {tTopics.map(topic => (
                                                                <tr key={topic.id} className="table-row-nested" style={{ backgroundColor: '#fafafa' }}>
                                                                    ...
                                                                    <td>
                                                                        <div className="admin-topic-cell" style={{ paddingLeft: '56px' }}>
                                                                            ...
                                                                            <div>
                                                                                <div className="admin-topic-name" style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.85rem', flexWrap: 'wrap' }}>
                                                                                    {topic.name}
                                                                                    {topic.isPublic && (
                        ...
                        <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                ...
                                <span style={{
                                    ...
                                    {deletedTopics.length + deletedFolders.length}
                                </span>
                            </div>
                            ...
                            <div className="admin-table-container">
                                <div className="trash-items-list" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                    {deletedFolders.map(folder => {
                                        ...
                                        return (
                                            <div key={`df-${folderId}`} className="trash-item-card" style={{
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '8px', borderTop: '1px solid #f1f5f9', paddingTop: '12px' }}>
                                                    <button className="admin-btn admin-btn-secondary" style={{ padding: '0 16px', fontSize: '0.8rem', height: '36px', borderRadius: '10px' }} disabled={trashActionLoading === folderId} onClick={async () => {
                                                        setTrashActionLoading(folderId);
                                                        try { await restoreTeacherTopicFolder(folderId); loadData(true); setAlertMessage({ type: 'success', text: 'Đã khôi phục folder!' }); } catch (e) { setAlertMessage({ type: 'error', text: e.message }); }
                                                        setTrashActionLoading(null);
                                                    }}>
                                                        <RotateCcw size={16} /> <span style={{ marginLeft: '6px' }}>Khôi phục</span>
                                                    </button>
                                                    <button className="admin-btn" style={{ padding: '0 16px', fontSize: '0.8rem', height: '36px', borderRadius: '10px', background: '#fff1f2', color: '#e11d48', border: '1px solid #fda4af' }} disabled={trashActionLoading === folderId} onClick={async () => {
                                                        if (!window.confirm('Xóa vĩnh viễn folder này?')) return;
                                                        setTrashActionLoading(folderId);
                                                        try { await permanentlyDeleteTeacherTopicFolder(folderId); loadData(true); setAlertMessage({ type: 'success', text: 'Xóa vĩnh viễn thành công!' }); } catch (e) { setAlertMessage({ type: 'error', text: e.message }); }
                                                        setTrashActionLoading(null);
                                                    }}>
                                                        <Trash2 size={16} /> <span style={{ marginLeft: '6px' }}>Xóa bỏ</span>
                                                    </button>
                                                </div>
                                            </div>
                                        );
                                    ...
                                    {deletedTopics.map(topic => {
                                        ...
                                        return (
                                            <div key={`dt-${topic.id}`} className="trash-item-card" style={{
                                                background: '#fff', borderRadius: '16px', padding: '16px',
                                                border: '1px solid #fecaca', boxShadow: '0 2px 6px rgba(0,0,0,0.02)',
                                                display: 'flex', flexDirection: 'column', gap: '12px'
                                            }}>
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, minWidth: 0 }}>
                                                        <div style={{
                                                            width: '40px', height: '40px', borderRadius: '12px',
                                                            background: `${topic.color || '#3b82f6'}15`, color: topic.color || '#3b82f6',
                                                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                                                        ...
                                                        <div style={{ minWidth: 0, flex: 1 }}>
                                                            <div style={{ fontWeight: 800, fontSize: '1rem', color: '#1e293b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{topic.name}</div>
                                                            <div style={{ fontSize: '0.8rem', color: '#64748b', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                                <User size={13} /> {teacher.displayName || teacher.email || 'Giáo viên'}
                                                            </div>
                                                        </div>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '8px', borderTop: '1px solid #f1f5f9', paddingTop: '12px' }}>
                                                    <button className="admin-btn admin-btn-secondary" style={{ padding: '0 16px', fontSize: '0.8rem', height: '36px', borderRadius: '10px' }} disabled={trashActionLoading === topic.id} onClick={async () => {
                                                        setTrashActionLoading(topic.id);
                                                        try { await restoreTeacherTopic(topic.id); loadData(true); setAlertMessage({ type: 'success', text: 'Đã khôi phục cho giáo viên!' }); } catch (e) { setAlertMessage({ type: 'error', text: e.message }); }
                                                        setTrashActionLoading(null);
                                                    }}>
                                                        <RotateCcw size={16} /> <span style={{ marginLeft: '6px' }}>Khôi phục</span>
                                                    </button>
                                                    <button className="admin-btn admin-btn-secondary" style={{ padding: '0 16px', fontSize: '0.8rem', height: '36px', borderRadius: '10px', color: '#7c3aed', borderColor: '#ddd6fe' }} disabled={trashActionLoading === topic.id} onClick={async () => {
                                                        setTrashActionLoading(topic.id);
                                                        ...
                                                        <RotateCcw size={16} /> <span style={{ marginLeft: '6px' }}>Về Admin</span>
                                                    </button>
                                                    <button className="admin-btn" style={{ padding: '0 16px', fontSize: '0.8rem', height: '36px', borderRadius: '10px', background: '#fff1f2', color: '#e11d48', border: '1px solid #fda4af' }} disabled={trashActionLoading === topic.id} onClick={async () => {
                                                        if (!window.confirm('Xóa vĩnh viễn bài học này? Tất cả từ vựng bên trong sẽ bị xóa.')) return;
                                                        setTrashActionLoading(topic.id);
                                                        try { await permanentlyDeleteTeacherTopic(topic.id); loadData(true); setAlertMessage({ type: 'success', text: 'Xóa vĩnh viễn thành công!' }); } catch (e) { setAlertMessage({ type: 'error', text: e.message }); }
                                                        setTrashActionLoading(null);
                                                    }}>
                                                        <Trash2 size={16} /> <span style={{ marginLeft: '6px' }}>Xóa bỏ</span>
                                                    </button>
                    ...
                    <div className="teacher-modal wide" style={{ maxWidth: '600px', overflow: 'auto' }}>
                        ...
                        <form onSubmit={handleTopicSubmit}>
                            <div className="admin-form-group">
                                ...
                                <input type="text" className="admin-form-input" required value={topicFormData.name || ''} onChange={e => setTopicFormData({ ...topicFormData, name: e.target.value })} />
                            </div>
                            <div className="admin-form-group">
                                <label>Mô tả</label>
                                <textarea className="admin-form-input admin-form-textarea" value={topicFormData.description || ''} onChange={e => setTopicFormData({ ...topicFormData, description: e.target.value })} />
                            </div>
                            ...
                            <div className="admin-modal-actions" style={{ marginTop: '24px', flexDirection: 'row' }}>
                                ...
                                <button type="submit" className="admin-btn admin-btn-primary" style={{ flex: 1 }} disabled={isSavingTopic}>{isSavingTopic ? 'Đang lưu...' : 'Cập nhật'}</button>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div className="teacher-modal-overlay">
                        <div className="teacher-modal">
                            ...
                            <p className="admin-modal-desc">
                                Bạn có chắc chắn muốn xóa bài học <strong>{topicToDelete.name}</strong>?<br/><br/>
                                <strong>Lưu ý:</strong> Toàn bộ từ vựng bên trong sẽ bị xóa vĩnh viễn và không thể khôi phục.
                            </p>
                            <div className="admin-modal-actions" style={{ flexDirection: 'row' }}>
                                <button className="admin-btn admin-btn-secondary" style={{ flex: 1 }} onClick={() => setTopicToDelete(null)} disabled={isDeleting}>Hủy</button>
                    ...
                    <div className="teacher-modal-overlay">
                        <div className="teacher-modal">
                            ...
                            <p className="admin-modal-desc">
                                ...
                                <strong>Lưu ý:</strong> Các bài học bên trong sẽ không bị xóa, chỉ folder bị xóa.
                            </p>
                            <div className="admin-modal-actions" style={{ flexDirection: 'row' }}>
